...
class TempFileHandler:
    ...
    def _add_private_entity_parameters(self, entity_name, entity_data, tn_descriptor, report_directory, tn_id):
        ...
        if entity_type == "tn_bastion":
            entity_input["one_component_networks"] = [280] + self.find_vxlan_ids(entity_input["one_component_networks"], tn_descriptor, report_directory, tn_id)
            ...
            entity_input["one_component_networks"] = [0] + self.find_vxlan_ids(entity_input["one_component_networks"], tn_descriptor, report_directory, tn_id)
        elif entity_type == "k8s_medium":
            entity_input["external_vnet_id"] = self.find_vxlan_ids(entity_input["external_vnet_id"], tn_descriptor, report_directory, tn_id)
            entity_input["internal_vnet_id"] = self.find_vxlan_ids(entity_input["internal_vnet_id"], tn_descriptor, report_directory, tn_id)
        elif entity_type == "ueransim":
            entity_input["one_component_networks"] = self.find_vxlan_ids(entity_input["one_component_networks"], tn_descriptor, report_directory, tn_id)
        return entity_input
