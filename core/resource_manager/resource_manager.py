...
class ResourceManagerHandler():
    ...
    def __init__(self, trial_network, sixg_sandbox_sites_handler):
        ...
        for _, entity_data in tn_descriptor.items():
            ...
            if sixg_sandbox_sites_component_quantity > 0:
                ...
                if not tnlcm_component_resources:
                    tnlcm_component_resources = ResourceManagerModel(site=self.trial_network.deployment_site, tn_ids=[self.trial_network.tn_id], component=component_type, quantity=1, ttl=sixg_sandbox_sites_component_ttl)
                else:
                    tnlcm_component_resources.tn_ids.append(self.trial_network.tn_id)
                    tnlcm_component_resources.quantity += 1
                tnlcm_component_resources.save()
    
    def release_resource_manager(self): # TODO
