    ...
    def post(self):
        ...
        try:
            ...
            if validate == "False":
                ...
                trial_network.save()
                TrialNetworkLogger(tn_id=tn_id).info(
                    message="Trial network validated. In this state, the trial network has been validated and is ready to be deployed"
                )
    ...
    def put(self, tn_id: str):
        ...
        try:
            ...
            if state == "validated":
                ...
                return {
                    "message": f"Trial network with identifier {tn_id} is not possible to activate. Only trial networks with status validated can be activated"
                }, 400
            elif state == "activated":
                return {"message": "Trial network is already activated"}, 400
            elif state == "suspended":
                # TODO:
                return {"message": "TODO: RESTART trial network suspended"}, 400
            else:
                return {"message": "Unknown state"}, 400
        except CustomException as e:
            trial_network.set_state(state="pending-activation")
            trial_network.save()
            TrialNetworkLogger(tn_id=tn_id).info(
                message="Trial network pending-activation. In this state, the trial network is waiting to be deployed"
            )
            return {"message": str(e)}, e.status_code
        except Exception as e:
            trial_network.set_state(state="pending-activation")
...
class DestroyTrialNetwork(Resource):
    ...
    def delete(self, tn_id: str):
        ...
        try:
            ...
            if (
                trial_network.state != "activated"
                and trial_network.state != "destroyed"
                and trial_network.state != "pending-activation"
                and trial_network.state != "pending-destruction"
            ):
                return {
                    "message": f"Trial network with identifier {tn_id} is not possible to destroy. Only trial networks with status activated, destroyed, pending-activation or pending-destruction can be destroyed"
                }, 400
            library_handler = LibraryHandler(
            ...
            trial_network.set_state(state="pending-destruction")
            trial_network.save()
            TrialNetworkLogger(tn_id=tn_id).info(
...
class PurgeTrialNetwork(Resource):
    ...
    def delete(self, tn_id: str):
        ...
        try:
            ...
            if (
                trial_network.state != "validated"
                and trial_network.state != "destroyed"
                and trial_network.state != "created"
            ):
                return {
                    "message": f"Trial network with identifier {tn_id} is not possible to purge. Only trial networks with status validated, destroyed or created can be purged"
                }, 400
    ...
    def get(self, tn_id: str):
        ...
        try:
            ...
            if trial_network.state != "activated":
                return {
                    "message": f"Trial network with identifier {tn_id} is not possible to retrieve the report. Only trial networks with status activated can retrieve the report"
                }, 400
...
class DownloadReportTrialNetwork(Resource):
    ...
    def get(self, tn_id: str):
        ...
        try:
            ...
            if trial_network.state != "activated":
                return {
                    "message": f"Trial network with identifier {tn_id} is not possible to download the report. Only trial networks with status activated can download the report"
                }, 400
...
class UpdateTrialNetwork(Resource):
    ...
    def put(self, tn_id: str):
        ...
        try:
            ...
            if state != "created":
                return {
                    "message": f"Trial network with identifier {tn_id} is not possible to update. Only trial networks with status created can be updated"
                }, 400
            trial_network.set_raw_descriptor(file=descriptor_file)
            trial_network.save()
