...
@trial_network_namespace.route("/create-validate")
class CreateValidateTrialNetwork(Resource):
    ...
    parser_post.add_argument("sites_branch", type=str, required=True)
    ...
    def post(self):
        ...
        Can specify only a branch of the Sites
        If desired the tn_id can be specified. **If is specified, it should begin with character and max length 15. If nothing is specified, it will return a random tn_id**
        """
        tn_log_handler = None
        ...
        try:
            ...
            if tn_id:
                ...
                if trial_network:
                    if trial_network.user_created != current_user.username:
                        return {"message": f"Trial network with identifier {trial_network.tn_id} was not created by the user {current_user.username}"}, 400
                    if trial_network.state != "created":
                        return {"message": f"Trial network with identifier {trial_network.tn_id} is not in status CREATED"}, 400
                    ...
                    trial_network.set_directory_path(directory_path=join_path(TnlcmSettings.TRIAL_NETWORKS_DIRECTORY, trial_network.tn_id))
                    tn_log_handler = TnLogHandler(tn_id=trial_network.tn_id)
                ...
                tn_log_handler.info(f"[{trial_network.tn_id}] - Create directory {trial_network.tn_id} in the path {trial_network.directory_path} to store the directories and files generated by the trial network")
                trial_network.set_raw_descriptor(file=descriptor_file)
            sites_handler = SitesHandler(https_url=sites_https_url, reference_type="branch", reference_value=sites_branch, directory_path=trial_network.directory_path)
            ...
            sites_handler.validate_sites_branch()
            ...
            tn_log_handler.info(f"[{trial_network.tn_id}] - Validate deployment site {sites_branch}")
            ...
            tn_log_handler.info(f"[{trial_network.tn_id}] - Deployment site {deployment_site} is valid")
...
@trial_network_namespace.route("/create")
class CreateTrialNetwork(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            tn_log_handler.info(f"[{trial_network.tn_id}] - Create directory {trial_network.tn_id} in the path {trial_network.directory_path} to store the directories and files generated by the trial network")
            trial_network.set_raw_descriptor(file=descriptor_file)
            trial_network.set_state("created")
            trial_network.save()
            tn_log_handler.info(f"[{trial_network.tn_id}] - Trial network update to status {trial_network.state}")
    ...
    def put(self, tn_id: str) -> tuple[dict, int]:
        ...
        try:
            ...
            if state == "validated":
                ...
                report_path = join_path(trial_network.directory_path, f"{trial_network.tn_id}.md")
                ...
                return {"message": "Trial network cannot be activated because the current status of Trial Network is CREATED and is not validated"}, 400
...
class PurgeTrialNetwork(Resource):
    ...
    def delete(self, tn_id: str) -> tuple[dict, int]:
        ...
        try:          
            ...
            if trial_network.state != "validated" and trial_network.state != "destroyed" and trial_network.state != "created":
...
class DownloadReportTrialNetwork(Resource):
    ...
    def get(self, tn_id: str):
        ...
        try:
            ...
            if not exists_path(path=report_path_md):
...
class TrialNetworkLog(Resource):
    ...
    def get(self, tn_id):
        ...
        try:
            ...
            if not exists_path(path=log_path):
