...
class CreateTrialNetwork(Resource):
    ...
    def post(self) -> tuple[dict, int]:
        ...
        try:
            ...
            tn_directory_path = os.path.join(TnlcmSettings.TRIAL_NETWORKS_DIRECTORY, f"{trial_network.tn_id}")
            trial_network.set_tn_directory_path(tn_directory_path)
            log_handler.info(f"[{trial_network.tn_id}] - Create the folder '{trial_network.tn_id}' in the path '{trial_network.tn_directory_path}' to store the directories and files generated by the trial network")
            ...
            sixg_sandbox_sites_handler = SixGSandboxSitesHandler(reference_type=github_6g_sandbox_sites_reference_type, reference_value=github_6g_sandbox_sites_reference_value, tn_directory_path=tn_directory_path)
            sixg_sandbox_sites_handler.git_clone()
            log_handler.info(f"[{trial_network.tn_id}] - Git clone '{sixg_sandbox_sites_handler.github_6g_sandbox_sites_repository_name}' repository into '{trial_network.tn_directory_path}'")
            sixg_sandbox_sites_handler.git_checkout()
            log_handler.info(f"[{trial_network.tn_id}] - Git checkout '{sixg_sandbox_sites_handler.github_6g_sandbox_sites_repository_name}' repository into '{trial_network.tn_directory_path}' to '{sixg_sandbox_sites_handler.github_6g_sandbox_sites_reference_type}' with value '{sixg_sandbox_sites_handler.github_6g_sandbox_sites_reference_value}'")
            sixg_sandbox_sites_handler.git_switch()
            log_handler.info(f"[{trial_network.tn_id}] - Git switch '{sixg_sandbox_sites_handler.github_6g_sandbox_sites_repository_name}' repository into '{trial_network.tn_directory_path}' with HEAD pointing to commit '{sixg_sandbox_sites_handler.github_6g_sandbox_sites_commit_id}'")
            ...
            sixg_library_handler = SixGLibraryHandler(reference_type=github_6g_library_reference_type, reference_value=github_6g_library_reference_value, tn_directory_path=tn_directory_path)
            sixg_library_handler.git_clone()
            log_handler.info(f"[{trial_network.tn_id}] - Git clone '{sixg_library_handler.github_6g_library_repository_name}' repository into '{trial_network.tn_directory_path}'")
            sixg_library_handler.git_checkout()
            log_handler.info(f"[{trial_network.tn_id}] - Git checkout '{sixg_library_handler.github_6g_library_repository_name}' repository into '{trial_network.tn_directory_path}' to '{sixg_library_handler.github_6g_library_reference_type}' with value '{sixg_library_handler.github_6g_library_reference_value}'")
            sixg_library_handler.git_switch()
            log_handler.info(f"[{trial_network.tn_id}] - Git switch '{sixg_library_handler.github_6g_library_repository_name}' repository into '{trial_network.tn_directory_path}' with HEAD pointing to commit '{sixg_library_handler.github_6g_library_commit_id}'")
    ...
    def put(self, tn_id: str) -> tuple[dict, int]:
        ...
        try:
            ...
            if tn_state == "validated":
                ...
                trial_network.save()
                sixg_sandbox_sites_handler = SixGSandboxSitesHandler(reference_type="commit", reference_value=trial_network.github_6g_sandbox_sites_commit_id, tn_directory_path=trial_network.tn_directory_path)
                ...
                trial_network.set_tn_report(os.path.join(f"{trial_network.tn_directory_path}", f"{trial_network.tn_id}.md"))
                trial_network.set_tn_state("activated")
                trial_network.save()
                log_handler.info(f"[{trial_network.tn_id}] - Trial network update to status '{trial_network.tn_state}'")
                return {"message": f"Trial network ACTIVATED. Report of the trial network can be found in the directory '{trial_network.tn_directory_path}/{trial_network.tn_id}.md'"}, 200
            elif tn_state == "failed":
                jenkins_handler = JenkinsHandler(trial_network=trial_network)
                tn_deployed_descriptor = trial_network.tn_deployed_descriptor["trial_network"]
                first_key = next(iter(tn_deployed_descriptor))
